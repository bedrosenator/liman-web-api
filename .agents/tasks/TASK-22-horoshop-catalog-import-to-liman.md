...
3. **Шаг 3: Добавление нового товара в Limansoft (если товар уникален)**:
   - **Автоматическое построение иерархии категорий в `name` (`LimanService.resolveCategoryGroup`)**:
     - Парсинг пути категорий вида `"Електроніка/Смартфони/iPhone 13"` (разделители `/` или `>`).
     - Рекурсивный поиск/создание узлов от корня к листьям.
     - Корневые группы получают свободные 2-значные коды (`'30'`, `'31'`, ...) с `parent = ''`.
     - Дочерние категории получают иерархический код с префиксом родителя (`'3001'`, `'300101'`) с лимитом `char(10)`.
     - Заполняются обязательные служебные поля Limansoft: `group_t: 0, stat: '', proc1..4: 0, cena: 0, cena_z: 4`.
...
4. **Шаг 4: Специфика интеграции с API Хорошоп**:
   - Сессионный токен из `/api/auth/` отправляется **только** в теле JSON (`{ "token": "..." }`). Передача заголовка `Authorization: Bearer` приводит к ошибке `AUTHORIZATION_ERROR: Incorrect Bearer Token` с кодом HTTP 200.
   - Все ответы Хорошопа проверяются по полю `status` (`OK`, `ERROR`, `AUTHORIZATION_ERROR`), так как HTTP-код практически всегда равен 200 OK.
   - Нормализация локализованных структур каталога (`title.ua`, `parent.value`, `stock`/`quantity`).
...
- [x] Добавить очередь BullMQ `import-horoshop-catalog` и процессор `HoroshopImportProcessor`.
- [x] Реализовать метод получения каталога из Хорошоп пачками через `POST /api/catalog/export/`.
- [x] Реализовать сервис сопоставления и вставки новых товаров в MariaDB с транзакциями:
  - Выделение `tcod`, создание категорий в `name`, запись в `name2`, `name2ost`, `namedesc`, `strihcod`.
- [x] Автоматическое наполнение таблицы `product_mappings` для `integrationId`.
- [x] Эндпоинт `POST /api/v1/horoshop/:tenantId/import/catalog`:
  - Параметры: `{ integrationId, mode: 'only_new' | 'overwrite', updatePrices, updateStock, updateImages, createBackup }`.
- [x] Эндпоинт отслеживания прогресса `GET /api/v1/sync/jobs/import-horoshop-catalog/:jobId`.

### 2. Пользовательский интерфейс (Frontend)
- [x] Выбор конкретной интеграции/магазина, если у клиента их несколько.
- [x] Карточка «Импорт товаров из Хорошоп в Limansoft» в кабинете клиента:
...
## 🧪 Критерии приемки
- [x] Импорт успешно выкачивает каталог тестового магазина `shop724088.horoshop.ua`.
- [x] Создаются записи в `product_mappings` для каждой импортированной позиции.
- [x] При повторном запуске в режиме "только новые" дубликаты не создаются.
- [x] Цены и остатки в MariaDB Limansoft обновляются только при явном выборе соответствующего режима.
- [x] Все тесты бэкенда и фронтенда проходят успешно.
