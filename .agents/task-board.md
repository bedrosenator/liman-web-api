...
- [x] **[TASK-40]** Исправление дефектов UI клиентского портала (бордеры табов, аккордеонов, бейджи, переполнение health-cards, верстка модалок завершения):
  - **Статус готовности**: ✅ **Выполнено и задеплоено на прод** (193/193 Jest, 69/69 Vitest);
  - **Приоритет**: 🔴 **Высший (UI/UX Bugfix)**;
  - **Подзадачи**:
    - [x] Заменить текстовую кнопку «Проверить связь» на `btn-icon--sm` в `PromHealthGrid`, `RozetkaTab` и `WooCommerceTab`;
    - [x] Сбросить системные стили заголовка `PromWizard` и убрать нижний бордер в закрытом состоянии;
    - [x] Убрать бордер у `.badge--subtle` и разделитель `border-b` под табами каналов;
    - [x] Добавить `min-w-0` к блокам с `truncate` (URL фида, вебхука) для защиты от распирания карточек;
    - [x] Оформить бейдж со свечением `.warning-badge-glow`, выровнять отступы и типографику в `ExportCompletedScreen` и `ImportCompletedScreen`;
    - [x] Обновить базу знаний `patterns_and_pitfalls.md` и правила `AGENTS.md`;
    - [x] Деплой на Hetzner прод сервер (188.245.254.12) с верификацией хелсчеков и ассетов.
