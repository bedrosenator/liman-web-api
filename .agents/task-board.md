...
- [ ] **[TASK-28]** Связывание артикулов (ID) Limansoft с Хорошоп и выгрузка каталога с категориями:
  - **Статус готовности**: 🟢 Анализ причин завершен (ошибка Хорошопа: *"Категория не найдена"* при создании новых товаров);
  - **Приоритет**: 🔴 **Критический (Блокирует экспорт товаров в Хорошоп)**;
  - **Подзадачи**:
    - Передача обязательных полей `title` и `parent` (категория) при выгрузке каталога в Хорошоп;
    - 3 стратегии сопоставления артикулов (ID): номенклатурный номер `nnom`, таблица маппинга по штрихкоду/названию, привязка через доп. поля;
    - Настройка категории по умолчанию для новых позиций;
    - Парсинг ответа `/api/catalog/import/` (`response.log`) с показом реального статуса (добавлено / обновлено / отклонено).
- [ ] **[TASK-21]** Модули интеграций для маркетплейсов Prom.ua, Rozetka и WooCommerce в Личном Кабинете:
  - **Статус готовности**: 🟢 100% готов к разработке (Бэкенд API, фиды, ping, sync готовы; дизайн-система и паттерны отработаны на Хорошоп);
  - **Приоритет**: 🔴 **Высший (Очередь #1)**;
  - **Подзадачи**:
    - Интеграция с Prom.ua в UI (статус связи по `ping`, форма API токена, копирование ссылки на YML-фид, кнопка «Синхронизировать остатки и цены», блок вебхука заказов);
    - Интеграция с Rozetka в UI (проверка Seller API, ввод Client ID/Secret, ссылка на XML прайс-лист, кнопка обновления остатков/цен);
    - Интеграция с WooCommerce в UI (проверка REST API по `ping`, ввод ключей ck/cs, тумблер Two-Way Sync, прямая кнопка скачивания плагина `limansoft-sync-woocommerce.zip`);
    - Витрина интеграций с бейджами «Подключено» / «Доступно для подключения» (Upsell);
    - Полная локализация интерфейсов на RU / UK через `LanguageContext`;
    - Vitest компонентные тесты новых вкладок интеграций в `client/src/test/`.
...
- [x] **[TASK-26]** Динамическое название магазина Хорошоп и прямой экспорт каталога в 1 клик:
  - Устранение хардкода `Columb Shop (Локальная MariaDB)` в шапке Клиентского портала;
  - Приоритетный вывод реального названия магазина из Хорошоп (`horoshopShopTitle` / `horoshopDomain`);
  - Кнопка прямого экспорта товаров в Хорошоп (`btn-direct-export`) в Action Hub без ручной возни с копированием ссылок на фид.
