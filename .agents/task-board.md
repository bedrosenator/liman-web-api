...
- [ ] **[TASK-29]** Единый изолированный движок заказов Limansoft MariaDB и интеграция Хорошоп:
  - **Статус готовности**: 🟢 100% готов к разработке (ТЗ и план утверждены в implementation_plan.md);
  - **Приоритет**: 🔴 **Высший (Очередь #1)**;
  - **Подзадачи**:
    - Изоляция единой бизнес-логики заказов в `LimanOrderService` (или `LimanService.processIncomingOrder`): единая точка входа для всех каналов (Хорошоп, WooCommerce, Prom, Rozetka);
    - DTO унифицированного заказа `UnifiedIncomingOrderDto` (клиент, доставка, оплата, позиции, источник);
    - Универсальный резолв товаров: строковые артикулы `ELE-...`, штрихкоды и маппинги ➔ `tcod` Лимана;
    - Радиокнопка/тумблер `horoshopCreateOrderDocumentEnabled` в настройках тенанта и UI (по умолчанию: **выключено**, пометка: «Экспериментальная функция»);
    - Безопасный Режим 1 (По умолчанию): атомарный декремент остатка `name2ost.skl_k` без вмешательства в таблицы документов;
    - Безопасный Режим 2 (Экспериментальный): создание черновика заказа `tip_dok: 85` (`prov: 'f'`), блокировка счетчика `ndok`, запись `checkdok`, строки `nakltelo` и аудит `dmonitor`;
    - Исправление фильтра статуса в Horoshop API (`stat_status: 1`);
    - Фиксация заказа №1 из Хорошоп в MariaDB.
