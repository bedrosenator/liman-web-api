...
5. **Автономный фоновый планировщик (`HoroshopSchedulerService`)**:
   ...
   - Если включена автосинхронизация (`horoshopExportEnabled = true`) и наступило время интервала (`horoshopSyncIntervalMinutes`: 5, 15, 30 или 60 минут), шедулер автоматически ставит задачу в очередь `sync-stock` (`targetPlatform: 'horoshop'`).
...
8. **Иерархическое дерево категорий и целевая категория по умолчанию (`defaultCategoryPath`)**:
   - **Ограничение Horoshop API**: метод `/api/catalog/import/` **не умеет** создавать категории «на лету» без готовых шаблонов. Если передать несуществующего родителя, Хорошоп отклоняет товар с ошибкой `код 7: "Категория не найдена, либо указан неверный шаблон"`.
   - **Динамическая загрузка категорий**: метод `HoroshopApiClient.getCatalogCategories` опрашивает `/api/pages/export/`, фильтрует страницы под корнем каталога (`parent: 97`) и строит полные пути категорий (например, `Електроніка/Смартфони/iPhone 13`, `Електроніка/Аксесуари`).
   - **API эндпоинт**: `GET /api/v1/horoshop/:tenantId/export/categories` возвращает клиенту актуальный список категорий для селектора в UI.
   - **Интеллектуальный маппинг в процессоре**: `HoroshopExportProcessor` пытается сопоставить группу товара из Limansoft с категорией Хорошопа по полному пути или названию. Если совпадения нет (или в базе Лимана указан локальный код группы вроде `0204`), товар автоматически помещается в выбранную пользователем целевую категорию `options.defaultCategoryPath`.

9. **Тонкости структуры логов API Хорошоп (`response.log`) и парсинг статусов**:
   - API Хорошоп не возвращает числовые агрегаты `created` / `updated` в корне ответа `/catalog/import/`. Результат всегда содержится в массиве `response.log`:
     ```json
     {
       "status": "OK",
       "response": {
         "log": [
           {
             "article": "16",
             "info": [
               { "code": 0, "message": "Товар с модификациями был перенесен в категорию Електроніка/Аксесуари..." },
               { "code": 11, "message": "Параметр barcode не найден" },
               { "code": 0, "message": "Товар обновлен" }
             ]
           }
         ]
       }
     ...
     ```
   - **Система кодов Хорошопа**:
     - `код 0` — **Успех**: действие выполнено (`"Товар добавлен"`, `"Товар обновлен"`, перенос модификаций и т.п.).
     - `код 7` — **Фатальная ошибка**: категория или шаблон не найдены, товар отклонён.
     - `код 11` — **Предупреждение шаблона**: переданный атрибут не зарегистрирован в шаблоне категории (например, штрихкод), но товар **успешно создан или обновлен**.
   - **Логика метода `parseHoroshopLogEntry`**:
     Проверяет наличие подтверждения `code === 0` и фатальных кодов (`code !== 0 && code !== 11`). Если код `0` присутствует и фатальных ошибок нет, позиция считается успешной, а предупреждения шаблона не приводят к ложным ошибкам выгрузки.

10. **Маппинг штрихкодов: `gtin` vs `barcode`**:
    - В каталоге Хорошоп системный атрибут для EAN/штрихкода называется **`gtin`**, а не `barcode`.
    - При передаче поля `barcode` Хорошоп возвращает предупреждение `код 11: "Параметр barcode не найден"`.
    - В `HoroshopExportProcessor.buildProductItem` штрихкод из базы Limansoft маппится в системное свойство `item.gtin = product.barcode`, что обеспечивает чистый ответ Хорошопа `status: "OK"` со статусом `код 0`.

11. **Управление жизненным циклом связей в `product_mappings`**:
    - Таблица `product_mappings` в PostgreSQL хранит сопоставления артикулов (`limanTcod` ➔ `externalArticle`).
    - Статус `synced` выставляется **только при реальном подтверждении успешного импорта** (`code === 0`).
    - Если товар отклонен Хорошопом (например, по коду 7), статус устанавливается в `error`, а в колонку `lastSyncError` записывается точный текст ошибки от Хорошоп. Это гарантирует, что проблемные товары не выпадут из последующих синхронизаций.
