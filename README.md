...
1. **Master API Key**: задается в `.env` (`MASTER_API_KEY`). Необходим для управления списком клиентов (`/api/v1/tenants`).
...
3. **Защита от IDOR/BOLA**: API-ключ тенанта строго изолирован и даёт доступ только к ресурсам со своим `tenantId`. Попытка обратиться к ресурсам другого клиента или получить глобальный список магазинов пресекается со статусом 401.
4. **Маскирование секретов**: при запросах к API все чувствительные поля (`dbPassword`, `woocommerceConsumerSecret`, `rozetkaClientSecret`, `horoshopPassword`) маскируются строкой `********`.
5. **HTTP Security Headers (Helmet)**:
   - Внедрены безопасные заголовки `Content-Security-Policy`, `Strict-Transport-Security` (HSTS), `X-Content-Type-Options: nosniff`, `X-Frame-Options: SAMEORIGIN`.
   - Настроен `Cross-Origin-Resource-Policy: cross-origin` для потоковой передачи изображений на сторонние витрины и маркетплейсы.
6. **Rate Limiting (Лимитирование нагрузки)**:
   - Подключен модуль `@nestjs/throttler` (по умолчанию: не более 120 запросов в минуту с одного IP-адреса).
   - Защищает от DoS-атак и флуда публичные эндпоинты стриминга фото, XML-фидов и приёма вебхуков.
7. **Ротация ключей**: в случае компрометации ключа выполните:
   ```bash
   POST /api/v1/tenants/:id/rotate-key
   ```
   Старый ключ аннулируется немедленно.
8. **Публичные эндпоинты (`@Public`)**: фиды XML (`feed.xml`), потоковая отдача медиа (`/media/*`) и вебхуки заказов (`/webhook/order`) доступны без передачи заголовка `x-api-key`, защищены лимитером запросов и фильтром исключений.
